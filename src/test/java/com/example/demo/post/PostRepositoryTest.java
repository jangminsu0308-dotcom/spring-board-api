...
class PostRepositoryTest {
    ...
    @Test
    void findByTitleContainingIgnoreCase_대소문자를_무시하고_부분일치로_검색한다() {
        ...
        Page<Post> result = postRepository.findByTitleContainingIgnoreCase(marker, PageRequest.of(0, 10));
    ...
    @Test
    void findByTitleContainingIgnoreCase_일치하는_게_없으면_빈_페이지를_반환한다() {
        em.persistAndFlush(new Post("제목", "내용", author));

        Page<Post> result = postRepository.findByTitleContainingIgnoreCase("MARKER-존재안함-" + UUID.randomUUID(), PageRequest.of(0, 10));
